...
#[tauri::command]
pub async fn save_settings(app_handle: tauri::AppHandle, settings: Value) -> Result<(), String> {
    let store = app_handle.store("settings.json").map_err(|e| e.to_string())?;
    store.set("app_settings".to_string(), settings);
    Ok(())
}

#[tauri::command]
pub async fn load_settings(app_handle: tauri::AppHandle) -> Result<Option<Value>, String> {
    let store = app_handle.store("settings.json").map_err(|e| e.to_string())?;
